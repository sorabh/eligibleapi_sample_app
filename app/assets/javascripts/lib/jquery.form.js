...
jQuery.fn.JSizedFormSelect = function (options) {
	return this.each(function () {
		jQuery(this).data('JSizedFormSelect', new JSized.Ui.Form.Select(this, options));
	});
};
...
JSized.Ui.Form.Select.prototype.styleElement = function () {
	...
	this.wrapper = jQuery('<div><ul></ul></div>').addClass(this.options.wrapperClass);
...
JSized.Ui.Form.Select.prototype.updateOptions = function () {
	this.optList.find('li').remove();
	this.optListEntries = jQuery.map(this.element[0].options, this.addOption.bind(this));
	this.selected.html(this.options.defaultText ? this.options.defaultText : jQuery(this.optListEntries[this.element[0].selectedIndex]).attr('data-display-text'));
...
JSized.Ui.Form.Select.prototype.addOption = function (element, i) {
	...
	var el = jQuery('<li />').addClass(element.hasClass('all') ? 'all' : '').addClass(disabled).attr({
